...
public class CandylandFruitDecorator extends CocoaDecorator {
	...
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("lucas:candyland_tree_fruit_decorator"), () -> DECORATOR_TYPE);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(Blocks.RED_CONCRETE.defaultBlockState(), direction1));
						}
					}
				}
			});
