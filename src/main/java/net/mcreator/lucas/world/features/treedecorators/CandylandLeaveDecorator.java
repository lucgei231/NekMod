...
@EventBusSubscriber
public class CandylandLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<CandylandLeaveDecorator> CODEC = MapCodec.unit(CandylandLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("lucas:candyland_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public CandylandLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.PINK_CONCRETE.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.PINK_CONCRETE.defaultBlockState(), direction));
